...
| `REGISTRY_AZURE_OIDC_AUDIENCE` | (empty) | Enables Microsoft Entra workload federation for explicitly approved Azure managed identities. Set this to the Application ID URI exposed by the registry API, for example `api://registry.agentics.dk`. Managed identities present their Entra access token as the `oauth2` Docker password; tenant, client and object IDs must all match a stored `azure` trust binding. |
