...
| `REGISTRY_AZURE_OIDC_AUDIENCE` | `fb60f99c-7a34-4190-8149-302f77469936` | Enables Microsoft Entra workload federation for explicitly approved Azure managed identities. This is the signed `aud` claim emitted when a managed identity requests `api://AzureADTokenExchange`; the default therefore works cross-tenant without a customer secret or per-tenant resource-app consent. A private deployment may configure its own Application ID URI audience, or set `off` to disable Azure federation. Tenant, client and object IDs must all match a stored `azure` trust binding. |
